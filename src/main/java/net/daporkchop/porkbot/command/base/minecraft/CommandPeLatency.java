...
public class CommandPeLatency extends Command {

    public CommandPeLatency() {
        super("pelatency");
    }
    ...
    public void execute(MessageReceivedEvent evt, String[] args, String message) {
        ...
        if (ipPort.length == 1) {
            ping = MCPing.pingPe(ipPort[0], 19132, true);
        } else if (ipPort.length == 2) {
            try {
                ping = MCPing.pingPe(ipPort[0], Integer.parseInt(ipPort[1]), true);
